...
use Fluxflow\Modules\Api\Models\ZhistFluxFluxes;

class ZhistfluxfluxesController extends ControllerBase 
{
    /*
    ...
    public function getAction($id)
    {
        ...
        {
            //Single record
            $record = ZhistFluxFluxes::findFirst($id);
            if( !$record) 
                $code = 404;
            else
            {
        ...
        {
            //Recordset to find
            $requestSetting = $this->parseSettings( $this->request, ZhistFluxFluxes );
            
            $struct = ZhistFluxFluxes::findStructured($requestSetting);
    ...
    public function newAction()
    {
        ...
        if(array_key_exists('flux_origins_id',$post))
        {
            if($post['flux_origins_id'] === "NULL")
            {
                $resource->flux_origins_id = NULL;
            }
            else 
            {
        ...
        if(array_key_exists('flux_status_id',$post))
        {
            if($post['flux_status_id'] === "NULL")
            {
                $resource->flux_status_id = NULL;
            }
            else 
            {
        ...
        if(array_key_exists('flux_priorities_id',$post))
        {
            if($post['flux_priorities_id'] === "NULL")
            {
                $resource->flux_priorities_id = NULL;
            }
            else 
            {
    ...
    public function updateAction($id)
    {
        ...
        $resource = ZhistFluxFluxes::findFirst($id);
        if( !$record )
            $code = 404;
        else 
        {
    ...
    public function indexAction()
    {
        ...
        $docFile = __DIR__ . "/docs/ZhistfluxfluxesController.php";
