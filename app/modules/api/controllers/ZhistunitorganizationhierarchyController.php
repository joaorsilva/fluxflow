...
use Fluxflow\Modules\Api\Models\ZhistUnitOrganizationHierarchy;

class ZhistunitorganizationhierarchyController extends ControllerBase 
{
    /*
    ...
    public function getAction($id)
    {
        ...
        {
            //Single record
            $record = ZhistUnitOrganizationHierarchy::findFirst($id);
            if( !$record) 
                $code = 404;
            else
            {
        ...
        {
            //Recordset to find
            $requestSetting = $this->parseSettings( $this->request, ZhistUnitOrganizationHierarchy );
            
            $struct = ZhistUnitOrganizationHierarchy::findStructured($requestSetting);
    ...
    public function newAction()
    {
        ...
        $resource = new ZhistUnitOrganizationHierarchy();
        ...
        if(array_key_exists('unit_parent_child_id',$post))
        {
            if($post['unit_parent_child_id'] === "NULL")
            {
                $resource->unit_parent_child_id = NULL;
            }
            else 
            {
                $resource->unit_parent_child_id = $post['unit_parent_child_id'];
            }
        }
        if(array_key_exists('direction',$post))
        {
            if($post['direction'] === "NULL")
            {
                $resource->direction = NULL;
            }
            else 
            {
                $resource->direction = $post['direction'];
            }
        }
        $resource->active = 1;
    ...
    public function updateAction($id)
    {
        ...
        $resource = ZhistUnitOrganizationHierarchy::findFirst($id);
        if( !$record )
            $code = 404;
        else 
        {
    ...
    public function indexAction()
    {
        ...
        $docFile = __DIR__ . "/docs/ZhistunitorganizationhierarchyController.php";
