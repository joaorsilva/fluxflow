...
use Fluxflow\Modules\Api\Models\ZhistFluxPriorities;

class ZhistfluxprioritiesController extends ControllerBase 
{
    /*
    ...
    public function getAction($id)
    {
        ...
        {
            //Single record
            $record = ZhistFluxPriorities::findFirst($id);
            if( !$record) 
                $code = 404;
            else
            {
        ...
        {
            //Recordset to find
            $requestSetting = $this->parseSettings( $this->request, ZhistFluxPriorities );
            
            $struct = ZhistFluxPriorities::findStructured($requestSetting);
    ...
    public function newAction()
    {
        ...
        $resource = new ZhistFluxPriorities();
        ...
        if(array_key_exists('unit_orgazinations_id',$post))
        {
            if($post['unit_orgazinations_id'] === "NULL")
            {
                $resource->unit_orgazinations_id = NULL;
            }
            else 
            {
                $resource->unit_orgazinations_id = $post['unit_orgazinations_id'];
            }
        }
        ...
        if(array_key_exists('priority_order',$post))
        {
            if($post['priority_order'] === "NULL")
            {
                $resource->priority_order = NULL;
            }
            else 
            {
                $resource->priority_order = $post['priority_order'];
            }
        }
        $resource->active = 1;
    ...
    public function updateAction($id)
    {
        ...
        $resource = ZhistFluxPriorities::findFirst($id);
        if( !$record )
            $code = 404;
        else 
        {
    ...
    public function indexAction()
    {
        ...
        $docFile = __DIR__ . "/docs/ZhistfluxprioritiesController.php";
