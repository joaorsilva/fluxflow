...
use Fluxflow\Modules\Api\Models\FluxStepFields;

class FluxstepfieldsController extends ControllerBase 
{
    /*
    ...
    public function getAction($id)
    {
        ...
        {
            //Single record
            $record = FluxStepFields::findFirst($id);
            if( !$record) 
                $code = 404;
            else
            {
        ...
        {
            //Recordset to find
            $requestSetting = $this->parseSettings( $this->request, FluxStepFields );
            
            $struct = FluxStepFields::findStructured($requestSetting);
    ...
    public function newAction()
    {
        ...
        if( 
            !$post
            || !isset($post['flux_field_types_id'])
            || !isset($post['unit_organizations_id'])
            || !isset($post['flux_subjects_id'])
            || !isset($post['flux_steps_id'])
            || !isset($post['val_type'])
            || !isset($post['field_caption'])
            || !isset($post['description_key'])
            || !isset($post['field_name'])
        ) 
            $code = 400;
        
        $resource = new FluxStepFields();
    ...
    public function updateAction($id)
    {
        ...
        $resource = FluxStepFields::findFirst($id);
        if( !$record )
            $code = 404;
        else 
        {
    ...
    public function indexAction()
    {
        ...
        $docFile = __DIR__ . "/docs/FluxstepfieldsController.php";
