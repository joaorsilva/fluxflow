...
use Fluxflow\Modules\Api\Models\ZhistUserRolePermissions;

class ZhistuserrolepermissionsController extends ControllerBase 
{
    /*
    ...
    public function getAction($id)
    {
        ...
        {
            //Single record
            $record = ZhistUserRolePermissions::findFirst($id);
            if( !$record) 
                $code = 404;
            else
            {
        ...
        {
            //Recordset to find
            $requestSetting = $this->parseSettings( $this->request, ZhistUserRolePermissions );
            
            $struct = ZhistUserRolePermissions::findStructured($requestSetting);
    ...
    public function newAction()
    {
        ...
        $resource = new ZhistUserRolePermissions();
        ...
        if(array_key_exists('app_modules_id',$post))
        {
            if($post['app_modules_id'] === "NULL")
            {
                $resource->app_modules_id = NULL;
            }
            else 
            {
        ...
        if(array_key_exists('app_controllers_id',$post))
        {
            if($post['app_controllers_id'] === "NULL")
            {
                $resource->app_controllers_id = NULL;
            }
            else 
            {
        ...
        if(array_key_exists('app_actions_id',$post))
        {
            if($post['app_actions_id'] === "NULL")
            {
                $resource->app_actions_id = NULL;
            }
            else 
            {
        ...
        if(array_key_exists('user_roles_id',$post))
        {
            if($post['user_roles_id'] === "NULL")
            {
                $resource->user_roles_id = NULL;
            }
            else 
            {
                $resource->user_roles_id = $post['user_roles_id'];
            }
        }
    ...
    public function updateAction($id)
    {
        ...
        $resource = ZhistUserRolePermissions::findFirst($id);
        if( !$record )
            $code = 404;
        else 
        {
    ...
    public function indexAction()
    {
        ...
        $docFile = __DIR__ . "/docs/ZhistuserrolepermissionsController.php";
