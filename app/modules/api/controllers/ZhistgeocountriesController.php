...
use Fluxflow\Modules\Api\Models\ZhistGeoCountries;

class ZhistgeocountriesController extends ControllerBase 
{
    /*
    ...
    public function getAction($id)
    {
        ...
        {
            //Single record
            $record = ZhistGeoCountries::findFirst($id);
            if( !$record) 
                $code = 404;
            else
            {
        ...
        {
            //Recordset to find
            $requestSetting = $this->parseSettings( $this->request, ZhistGeoCountries );
            
            $struct = ZhistGeoCountries::findStructured($requestSetting);
    ...
    public function newAction()
    {
        ...
        $resource = new ZhistGeoCountries();
        ...
        if(array_key_exists('code_1',$post))
        {
            if($post['code_1'] === "NULL")
            {
                $resource->code_1 = NULL;
            }
            else 
            {
                $resource->code_1 = $post['code_1'];
            }
        }
        if(array_key_exists('code_2',$post))
        {
            if($post['code_2'] === "NULL")
            {
                $resource->code_2 = NULL;
            }
            else 
            {
                $resource->code_2 = $post['code_2'];
            }
        }
        if(array_key_exists('phone_code',$post))
        {
            if($post['phone_code'] === "NULL")
            {
                $resource->phone_code = NULL;
            }
            else 
            {
                $resource->phone_code = $post['phone_code'];
            }
        }
        $resource->active = 1;
    ...
    public function updateAction($id)
    {
        ...
        $resource = ZhistGeoCountries::findFirst($id);
        if( !$record )
            $code = 404;
        else 
        {
    ...
    public function indexAction()
    {
        ...
        $docFile = __DIR__ . "/docs/ZhistgeocountriesController.php";
