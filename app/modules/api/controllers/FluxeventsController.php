...
use Fluxflow\Modules\Api\Models\FluxEvents;

class FluxeventsController extends ControllerBase 
{
    /*
    ...
    public function getAction($id)
    {
        ...
        {
            //Single record
            $record = FluxEvents::findFirst($id);
            if( !$record) 
                $code = 404;
            else
            {
        ...
        {
            //Recordset to find
            $requestSetting = $this->parseSettings( $this->request, FluxEvents );
            
            $struct = FluxEvents::findStructured($requestSetting);
    ...
    public function newAction()
    {
        ...
        if( 
            !$post
            || !isset($post['flux_event_types_id'])
            || !isset($post['flux_fluxes_id'])
            || !isset($post['flux_messages_id'])
        ) 
            $code = 400;
        
        $resource = new FluxEvents();
    ...
    public function updateAction($id)
    {
        ...
        $resource = FluxEvents::findFirst($id);
        if( !$record )
            $code = 404;
        else 
        {
    ...
    public function indexAction()
    {
        ...
        $docFile = __DIR__ . "/docs/FluxeventsController.php";
