...
use Fluxflow\Modules\Api\Models\ZhistFluxSteps;

class ZhistfluxstepsController extends ControllerBase 
{
    /*
    ...
    public function getAction($id)
    {
        ...
        {
            //Single record
            $record = ZhistFluxSteps::findFirst($id);
            if( !$record) 
                $code = 404;
            else
            {
        ...
        {
            //Recordset to find
            $requestSetting = $this->parseSettings( $this->request, ZhistFluxSteps );
            
            $struct = ZhistFluxSteps::findStructured($requestSetting);
    ...
    public function newAction()
    {
        ...
        if(array_key_exists('step_order',$post))
        {
            if($post['step_order'] === "NULL")
            {
                $resource->step_order = NULL;
            }
            else 
            {
                $resource->step_order = $post['step_order'];
            }
        }
        $resource->active = 1;
    ...
    public function updateAction($id)
    {
        ...
        $resource = ZhistFluxSteps::findFirst($id);
        if( !$record )
            $code = 404;
        else 
        {
    ...
    public function indexAction()
    {
        ...
        $docFile = __DIR__ . "/docs/ZhistfluxstepsController.php";
