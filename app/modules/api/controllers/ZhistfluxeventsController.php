...
use Fluxflow\Modules\Api\Models\ZhistFluxEvents;

class ZhistfluxeventsController extends ControllerBase 
{
    /*
    ...
    public function getAction($id)
    {
        ...
        {
            //Single record
            $record = ZhistFluxEvents::findFirst($id);
            if( !$record) 
                $code = 404;
            else
            {
        ...
        {
            //Recordset to find
            $requestSetting = $this->parseSettings( $this->request, ZhistFluxEvents );
            
            $struct = ZhistFluxEvents::findStructured($requestSetting);
    ...
    public function newAction()
    {
        ...
        if(array_key_exists('flux_event_types_id',$post))
        {
            if($post['flux_event_types_id'] === "NULL")
            {
                $resource->flux_event_types_id = NULL;
            }
            else 
            {
                $resource->flux_event_types_id = $post['flux_event_types_id'];
            }
        }
        ...
        if(array_key_exists('flux_messages_id',$post))
        {
            if($post['flux_messages_id'] === "NULL")
            {
                $resource->flux_messages_id = NULL;
            }
            else 
            {
                $resource->flux_messages_id = $post['flux_messages_id'];
            }
        }
        $resource->active = 1;
    ...
    public function updateAction($id)
    {
        ...
        $resource = ZhistFluxEvents::findFirst($id);
        if( !$record )
            $code = 404;
        else 
        {
    ...
    public function indexAction()
    {
        ...
        $docFile = __DIR__ . "/docs/ZhistfluxeventsController.php";
